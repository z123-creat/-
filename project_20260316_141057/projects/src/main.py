...
async def get_landing_page():
    """返回引导页面"""
    try:
        template_path = os.path.join(os.getenv("COZE_WORKSPACE_PATH", "."), "src/templates/landing.html")
        with open(template_path, 'r', encoding='utf-8') as f:
            html_content = f.read()
        return HTMLResponse(content=html_content)
    except Exception as e:
        logger.error(f"Error loading landing page: {e}")
        return HTMLResponse(content=f"<h1>服务运行中，但引导页面加载失败</h1><p>错误: {str(e)}</p>")
...
async def get_chat_page():
    """返回聊天界面"""
    try:
        template_path = os.path.join(os.getenv("COZE_WORKSPACE_PATH", "."), "src/templates/chat_with_token.html")
        with open(template_path, 'r', encoding='utf-8') as f:
            html_content = f.read()
        return HTMLResponse(content=html_content)
    except Exception as e:
        logger.error(f"Error loading chat interface: {e}")
        return HTMLResponse(content=f"<h1>服务运行中，但聊天界面加载失败</h1><p>错误: {str(e)}</p>")
